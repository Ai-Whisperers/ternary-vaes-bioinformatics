# Email Draft: Sergey Ovchinnikov

**Status:** READY TO SEND

**Email:** so3@mit.edu (verified via MIT)

---

## Recipient Research

| Field | Details |
|-------|---------|
| **Name** | Sergey Ovchinnikov |
| **Position** | Assistant Professor, Biology, MIT |
| **Key Work** | ColabFold, coevolution analysis, trRosetta contributions |
| **Recent Focus** | Making structure prediction accessible, coevolution signals |
| **Lab** | solab.org |

---

## EMAIL

**To:** so3@mit.edu

**Subject:** Coevolution signals in hyperbolic space?

---

Sergey,

Your work democratizing structure prediction (ColabFold!) and understanding coevolution signals is exactly what I want to ask about.

We're embedding sequences in hyperbolic space using p-adic structure and getting stability predictions (ρ=0.58 (N=52), N=52) without alignments. The question: is this capturing something related to coevolution, or something orthogonal?

The hyperbolic geometry enforces ultrametric-like distances (82.8% compliance), which feels related to phylogenetic relationships, but I'm not sure if that connects to the coevolution signal you've characterized.

Any intuition on whether these could be related or distinct signals?

Best,
Ivan Weiss Van Der Pol
Jonathan Verdun
Kyrian Weiss Van Der Pol
AI Whisperers | CONACYT Paraguay
github.com/Ai-Whisperers/ternary-vaes-bioinformatics

---

## Notes

- **Word count:** 104
- **Why this works:** References ColabFold (his popular work), asks about coevolution interpretation
- **Tone:** Enthusiastic peer

---

*Last Updated: 2026-01-26*
