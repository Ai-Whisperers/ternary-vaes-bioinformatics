...
class TensorAssertions:
    ...
    def assert_shape(
        ...
        if tensor.shape != expected_shape:
            raise AssertionError(
                f"Shape mismatch: expected {expected_shape}, got {tensor.shape}. {msg}"
            )
    ...
    def assert_device(tensor: torch.Tensor, device: str, msg: str = ""):
        ...
        if device not in actual_device:
            raise AssertionError(
                f"Device mismatch: expected {device}, got {actual_device}. {msg}"
            )
    ...
    def assert_dtype(tensor: torch.Tensor, dtype: torch.dtype, msg: str = ""):
        ...
        if tensor.dtype != dtype:
            raise AssertionError(
                f"Dtype mismatch: expected {dtype}, got {tensor.dtype}. {msg}"
            )
    ...
    def assert_finite(tensor: torch.Tensor, msg: str = ""):
        ...
        if not torch.isfinite(tensor).all():
            ...
            raise AssertionError(
                f"Tensor contains non-finite values: {nan_count} NaN, {inf_count} Inf. {msg}"
            )
    ...
    def assert_positive(tensor: torch.Tensor, msg: str = ""):
        ...
        if (tensor <= 0).any():
            ...
            raise AssertionError(
                f"Tensor contains non-positive values. Min: {min_val}. {msg}"
            )
    ...
    def assert_non_negative(tensor: torch.Tensor, msg: str = ""):
        ...
        if (tensor < 0).any():
            min_val = tensor.min().item()
            raise AssertionError(
                f"Tensor contains negative values. Min: {min_val}. {msg}"
            )
    ...
    def assert_in_range(
        ...
        if inclusive:
            if (tensor < low).any() or (tensor > high).any():
                ...
                raise AssertionError(
                    f"Values outside range [{low}, {high}]. "
                    f"Actual range: [{min_val}, {max_val}]. {msg}"
                )
        else:
            if (tensor <= low).any() or (tensor >= high).any():
                min_val, max_val = tensor.min().item(), tensor.max().item()
    ...
    def assert_close(
        ...
        if not torch.allclose(actual, expected, atol=atol, rtol=rtol):
            ...
            raise AssertionError(
                f"Tensors not close. Max diff: {max_diff}, atol: {atol}, rtol: {rtol}. {msg}"
            )
    ...
    def assert_equal(actual: torch.Tensor, expected: torch.Tensor, msg: str = ""):
        ...
        if not torch.equal(actual, expected):
            ...
            raise AssertionError(
                f"Tensors not equal. {diff_count} elements differ. {msg}"
            )
...
class GeometryAssertions:
    ...
    def assert_on_poincare_disk(
        ...
        if not (norms < max_norm + tolerance).all():
            ...
            raise AssertionError(
                f"Points outside Poincaré disk! Max norm: {max_val:.6f} >= {max_norm}. "
                f"{violations} violations. {msg}"
            )
    ...
    def assert_distance_identity(
        ...
        if not torch.allclose(dist_aa, torch.zeros_like(dist_aa), atol=atol):
            ...
            raise AssertionError(
                f"Identity distance violation: max self-distance {max_val}. {msg}"
            )
    ...
    def assert_triangle_inequality(
        ...
        if violations.any():
            ...
            raise AssertionError(
                f"Triangle inequality violated! {violation_count} violations. "
                f"Max violation: {max_violation}. {msg}"
            )
    ...
    def assert_radial_ordering(
        ...
        if inverse:
            if correlation > 0:
                raise AssertionError(
                    f"Expected negative correlation (inverse radial ordering). "
                    f"Got correlation: {correlation:.4f}. {msg}"
                )
        else:
            if correlation < 0:
                raise AssertionError(
                    f"Expected positive correlation (direct radial ordering). "
                    f"Got correlation: {correlation:.4f}. {msg}"
                )
...
class ModelAssertions:
    ...
    def assert_output_keys(outputs: Dict[str, Any], required_keys: list, msg: str = ""):
        ...
        if missing:
            raise AssertionError(
                ...
                f"Available keys: {list(outputs.keys())}. {msg}"
            )
    ...
    def assert_logits_shape(
        ...
        if logits.shape != expected:
            raise AssertionError(
                f"Logits shape mismatch: expected {expected}, got {logits.shape}. {msg}"
            )
    ...
    def assert_latent_shape(
        ...
        if z.shape != expected:
            raise AssertionError(
                f"Latent shape mismatch: expected {expected}, got {z.shape}. {msg}"
            )
            ...
            if param.grad is not None:
                if not torch.isfinite(param.grad).all():
                    ...
                    raise AssertionError(
                        ...
                        f"{nan_count} NaN, {inf_count} Inf. {msg}"
                    )
    ...
    def assert_loss_decreases(
        ...
        if strict:
            ...
            if losses[-1] > losses[0]:
                raise AssertionError(
                    f"Loss did not decrease overall: {losses[0]:.6f} -> {losses[-1]:.6f}. {msg}"
                )
...
class TernaryAssertions:
    ...
    def assert_valid_ternary(tensor: torch.Tensor, msg: str = ""):
        ...
        if invalid:
            raise AssertionError(
                f"Invalid ternary values: {invalid}. Expected only {valid_values}. {msg}"
            )
    ...
    def assert_ternary_shape(
        ...
        if len(tensor.shape) != 2:
            raise AssertionError(
                f"Expected 2D tensor, got {len(tensor.shape)}D. Shape: {tensor.shape}. {msg}"
            )

        if tensor.shape[1] != 9:
            raise AssertionError(
                f"Expected 9 digits, got {tensor.shape[1]}. Shape: {tensor.shape}. {msg}"
            )

        if batch_size is not None and tensor.shape[0] != batch_size:
            raise AssertionError(
                f"Expected batch size {batch_size}, got {tensor.shape[0]}. {msg}"
            )
