#!/usr/bin/env python3
"""Phase 2 sweep: Loss weight variations with curvature=2.0.

Based on Phase 1 findings, curvature=2.0 is marginally better.
Now test loss weight ratios to find optimal hierarchy/richness balance.

Experiments:
1. Baseline: hier=5, rich=2 (current best)
2. High hierarchy: hier=8, rich=1
3. High richness: hier=3, rich=4
4. Balanced: hier=5, rich=5
5. Extreme hierarchy: hier=10, rich=0.5

Usage:
    python scripts/experiments/epsilon_vae/sweep_phase2.py
"""
...
def run_experiment(config, all_ops, indices, device, base_ckpt_path):
    ...
    save_dir = PROJECT_ROOT / f"sandbox-training/checkpoints/sweep2_{exp_name}"
...
def main():
    ...
    # Phase 2 experiments: Loss weight variations
    ...
    print(f"Estimated time: {len(experiments) * 2} - {len(experiments) * 4} minutes per experiment")
    ...
    # Save summary
    summary_path = PROJECT_ROOT / "sandbox-training/checkpoints/sweep2_summary.json"
