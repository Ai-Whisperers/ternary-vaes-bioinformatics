...
# Load both v5.8 and v5.9
ckpt_v58 = torch.load('sandbox-training/checkpoints/v5_8/latest.pt', map_location='cpu', weights_only=False)
ckpt_v59 = torch.load('sandbox-training/checkpoints/v5_9/epoch_150.pt', map_location='cpu', weights_only=False)
...
ax.plot(epochs, corr_euc_59, label='v5.9 Euclidean', color='#e67e22', linewidth=2, linestyle='--')
...
if len(H_A_58) > 0:
    ax.plot(range(len(H_A_58)), H_A_58, label='v5.8 H_A', color='#3498db', linewidth=1.5)
    ax.plot(range(len(H_B_58)), H_B_58, label='v5.8 H_B', color='#e74c3c', linewidth=1.5)
if len(H_A_59) > 0:
    ax.plot(range(len(H_A_59)), H_A_59, label='v5.9 H_A', color='#9b59b6', linewidth=1.5, linestyle='--')
    ax.plot(range(len(H_B_59)), H_B_59, label='v5.9 H_B', color='#e67e22', linewidth=1.5, linestyle='--')
ax.set_xlabel('Epoch')
...
ax.fill_between(epochs, 0, diff, where=(diff > 0), alpha=0.3, color='green', label='Hyp > Euc')
ax.fill_between(epochs, 0, diff, where=(diff <= 0), alpha=0.3, color='red', label='Euc > Hyp')
...
"""
ax.text(0.5, 0.5, summary, transform=ax.transAxes, fontsize=11, verticalalignment='center',
        horizontalalignment='center', fontfamily='monospace', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
