...
# Load v5.9 epoch 150
ckpt = torch.load('sandbox-training/checkpoints/v5_9/epoch_150.pt', map_location='cpu', weights_only=False)
...
ax.plot(epochs, corr_hyp, label='Hyperbolic (Poincare Ball)', color='#9b59b6', linewidth=2.5)
ax.plot(epochs, corr_euc, label='Euclidean', color='#3498db', linewidth=2, linestyle='--')
ax.fill_between(epochs, corr_hyp, corr_euc, alpha=0.2, color='#9b59b6',
                where=(corr_hyp > corr_euc), label='Hyp advantage')
ax.axhline(0.7, color='gold', linestyle='--', alpha=0.7, label='0.7 threshold')
ax.axhline(max(corr_hyp), color='green', linestyle=':', alpha=0.5, label=f'Max: {max(corr_hyp):.4f}')
ax.set_xlabel('Epoch', fontsize=12)
ax.set_ylabel('3-adic Distance Correlation (r)', fontsize=12)
ax.set_title('v5.9: Hyperbolic vs Euclidean 3-adic Distance Correlation\n(Poincare Ball Geometry)', fontsize=14)
...
ax.annotate(f'Peak: {max(corr_hyp):.4f}\nEpoch {best_epoch}',
            xy=(best_epoch, max(corr_hyp)), xytext=(best_epoch + 10, max(corr_hyp) - 0.02),
...
ax.fill_between(range(len(smoothed)), 0, smoothed, where=(smoothed > 0), alpha=0.3, color='green')
ax.fill_between(range(len(smoothed)), 0, smoothed, where=(smoothed <= 0), alpha=0.3, color='red')
ax.set_xlabel('Epoch')
...
ax.scatter(phase1, corr_hyp[:50], s=20, alpha=0.6, label='Phase 1 (0-50)', color='#3498db')
ax.scatter(phase2, corr_hyp[50:100], s=20, alpha=0.6, label='Phase 2 (50-100)', color='#e67e22')
ax.scatter(phase3, corr_hyp[100:], s=20, alpha=0.6, label='Phase 3 (100-150)', color='#2ecc71')
ax.set_xlabel('Epoch')
...
ax.hist(corr_hyp, bins=30, alpha=0.7, color='#9b59b6', edgecolor='black', label='Hyperbolic')
ax.hist(corr_euc, bins=30, alpha=0.5, color='#3498db', edgecolor='black', label='Euclidean')
ax.axvline(np.mean(corr_hyp), color='#9b59b6', linestyle='--', linewidth=2, label=f'Hyp mean: {np.mean(corr_hyp):.4f}')
ax.axvline(np.mean(corr_euc), color='#3498db', linestyle='--', linewidth=2, label=f'Euc mean: {np.mean(corr_euc):.4f}')
...
ax.text(0.1, 0.95, summary, transform=ax.transAxes, fontsize=10, verticalalignment='top',
        fontfamily='monospace', bbox=dict(boxstyle='round', facecolor='#f0e68c', alpha=0.8))
