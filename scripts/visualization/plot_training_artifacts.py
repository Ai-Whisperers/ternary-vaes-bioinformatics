...
def plot_benchmark_summary(output_path):
    ...
    ax.set_ylim(98, 100)
    ax.legend()
    for bar, val in zip(bars, [coverage['vae_a_mean_coverage'], coverage['vae_b_mean_coverage']]):
    ...
    ax.axhline(np.log2(16), color='gray', linestyle='--', label=f'Max (log₂16={np.log2(16):.2f})')
    ...
    speeds = [inference['vae_a_samples_per_sec'] / 1e6,
    ...
    mem_vals = [memory['baseline_gb'] * 1000, memory['overhead_gb'] * 1000, memory['peak_gb'] * 1000]
    bars = ax.bar(['Baseline', 'Overhead', 'Peak'], mem_vals, color=['gray', 'orange', 'red'], alpha=0.8)
...
def plot_manifold_resolution(output_path):
    ...
    width = 0.35
    ax.bar([x - width/2 for x in x_A], y_A, width, label='VAE-A', color='#3498db', alpha=0.8)
    ax.bar([x + width/2 for x in x_B], y_B, width, label='VAE-B', color='#e74c3c', alpha=0.8)
    ...
    ax.annotate(f'Ratio: {ratio:.1f}x', xy=(1, sep_B['latent_norm_mean']),
    ...
    categories = ['reconstruction', 'coverage', 'interpolation', 'nearest_neighbor', 'dimensionality']
    N = len(categories)
    ...
    ax.plot(angles, vals_A, 'o-', linewidth=2, label=f"VAE-A ({scores_A['overall']:.2f})", color='#3498db')
    ax.fill(angles, vals_A, alpha=0.25, color='#3498db')
    ax.plot(angles, vals_B, 'o-', linewidth=2, label=f"VAE-B ({scores_B['overall']:.2f})", color='#e74c3c')
    ...
    ev_A = make_spectrum(dim_A['max_eigenvalue'], dim_A['min_eigenvalue'], dim_A['effective_dim'])
    ev_B = make_spectrum(dim_B['max_eigenvalue'], dim_B['min_eigenvalue'], dim_B['effective_dim'])
...
def plot_coupled_system(output_path):
    ...
    strategies = ['voting', 'confidence\nweighted', 'best_of_two', 'VAE-A\nalone', 'VAE-B\nalone']
    values = [ensemble['voting']['exact_match_rate'],
    ...
    rhos = ['ρ=0\n(VAE-A)', 'ρ=0\n(VAE-B)', 'ρ=0.5', 'ρ=0.7']
    coverages = [rho_05['baseline_vae_a_coverage'] * 100,
    ...
    categories = ['Both\nPerfect', 'VAE-A\nBest', 'VAE-B\nBest', 'Both\nImperfect']
    counts = [comp['both_perfect'], comp['vae_a_best'], comp['vae_b_best'], comp['both_imperfect']]
    ...
    ax.axvline(coupling['mean_distance'], color='red', linestyle='--',
               label=f'Mean: {coupling["mean_distance"]:.1f}')
    ax.axvline(coupling['min_distance'], color='green', linestyle=':',
               label=f'Min: {coupling["min_distance"]:.1f}')
    ax.axvline(coupling['max_distance'], color='orange', linestyle=':',
    ...
    values = [sys_res['ensemble'], sys_res['coverage'], sys_res['coupling'], sys_res['overall']]
    bars = ax.barh(categories, values, color=['green', 'blue', 'purple', 'gold'], alpha=0.8)
    ax.set_xlabel('Score')
...
def main():
    ...
    for f in sorted(output_path.glob('*.png')):
        if any(x in f.name for x in ['benchmark', 'manifold_resolution', 'coupled', 'checkpoint']):
            print(f"  - {f.name}")
