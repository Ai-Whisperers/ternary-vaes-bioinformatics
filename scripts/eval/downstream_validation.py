...
def load_model(checkpoint_path: Path, device: str = 'cuda'):
    ...
    trainable_state = {k: v for k, v in state.items()
                       if 'projection' in k or 'encoder_B' in k}
    if trainable_state:
...
def compute_production_readiness_score(nn_results, hierarchy_results, arithmetic_results):
    ...
    checks = {
        'NN Adjacent Rate': (vae_a_nn >= thresholds['nn_adjacent_rate'], vae_a_nn, thresholds['nn_adjacent_rate']),
        'Hierarchy Pairwise': (vae_a_pairwise >= thresholds['hierarchy_pairwise'], vae_a_pairwise, thresholds['hierarchy_pairwise']),
        'Hierarchy Spearman': (vae_a_spearman <= thresholds['hierarchy_spearman'], vae_a_spearman, thresholds['hierarchy_spearman']),
        'Arithmetic Prediction': (vae_a_arith >= thresholds['arithmetic_valuation'], vae_a_arith, thresholds['arithmetic_valuation']),
    }
...
def main():
    ...
    parser.add_argument('--k', type=int, default=10,
                        help='Number of neighbors for NN test')
