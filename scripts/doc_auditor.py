...
def audit_docs():
    ...
    ignore_dirs = {'.git', '.mypy_cache', '.pytest_cache', '.ruff_cache', '__pycache__', 'node_modules', 'venv', '.venv'}
    ...
    for root, dirs, files in os.walk("."):
        ...
        for file in files:
            if file.endswith((".md", ".txt")):
                ...
                elif any(kw in file_path.lower() for kw in ["theory", "math", "scientific", "research", "paper", "foundations", "alphafold"]):
                    inventory["Scientific"].append(file_path)
                elif any(kw in file_path.lower() for kw in ["setup", "api", "dev", "developer", "readme", "install", "tech", "workflow", "conductor", "rules", "scripts", "src", "tests"]):
