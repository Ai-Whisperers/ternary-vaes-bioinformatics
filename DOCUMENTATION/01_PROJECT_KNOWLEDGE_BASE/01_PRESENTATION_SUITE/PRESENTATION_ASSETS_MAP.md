# Master Presentation Assets Map

> **Objective**: To secure interest from "Important People" (VIPs) by having the _exact_ right document for every conversation.

---

## 🏛️ Audience A: The Academics (DeepMind, Oxford, Broad Institute)

**Goal**: Validation, Collaboration, Peer Review.
_They care about: Novelty, Rigor, Reproducibility._

| Asset Type           | Document Name                                                                   | Status       | Description                                                              |
| :------------------- | :------------------------------------------------------------------------------ | :----------- | :----------------------------------------------------------------------- |
| **Pre-print**        | `02_SCIENTIFIC_COMMUNICATION/THEORY_Technical_Whitepaper.md`                    | ✅ **Ready** | A unified, arXiv-style paper combining Math + Bio results (The "Paper"). |
| **Math Proofs**      | `../02_THEORY_AND_FOUNDATIONS/validation_suite/02_MATHEMATICAL_STRESS_TESTS.md` | ✅ **Ready** | The rigorous geometric proofs (validation_suite).                        |
| **Benchmarks**       | `../02_THEORY_AND_FOUNDATIONS/validation_suite/01_BIOLOGICAL_BENCHMARKS.md`     | ✅ **Ready** | Comparison vs. EVE/ProteinGym (validation_suite).                        |
| **Code Walkthrough** | `02_SCIENTIFIC_COMMUNICATION/THEORY_Code_Architecture.md`                       | ✅ **Ready** | "How to run the model yourself" (for skeptics).                          |
| **Theory Guide**     | `02_SCIENTIFIC_COMMUNICATION/THEORY_Math_Foundations/*.md`                      | ✅ **Ready** | The 3-part split guide (Formulation, StateNet, Convergence).             |

---

## 💊 Audience B: The Industry (Pfizer, Moderna, Dyno Tx)

**Goal**: Pilot Programs, Licensing, Acquisition.
_They care about: Efficiency, Scalability, Integration._

| Asset Type            | Document Name | Status       | Description                               |
| :-------------------- | :------------ | :----------- | :---------------------------------------- |
| **Executive Summary** | `PITCH.md`    | ✅ **Ready** | High-level "Hook" (Problem/Solution/Ask). |

## 💰 Audience C: The Investors (TechBio VCs, Grants)

**Goal**: Funding ($2M-$10M Seed).
_They care about: Market Size, Moat, Team, Exit._

| Asset Type            | Document Name                                                                 | Status       | Description                                           |
| :-------------------- | :---------------------------------------------------------------------------- | :----------- | :---------------------------------------------------- |
| **Pitch Deck Script** | `01_INVESTOR_PACKAGE/01_pitch_materials/PITCH_DECK_SCRIPT.md`                 | ✅ **Ready** | Slide-by-slide narrative for the 15-slide deck.       |
| **Market Analysis**   | `01_INVESTOR_PACKAGE/02_market_analysis/MARKET_OPPORTUNITY.md`                | ✅ **Ready** | TAM/SAM/SOM for "Generative Biology".                 |
| **Moat Definitions**  | `01_INVESTOR_PACKAGE/03_legal_and_ip/IP_AND_DEFENSIBILITY.md`                 | ✅ **Ready** | Why Google can't just copy us tomorrow (IP Strategy). |
| **Team & Roadmap**    | `../../02_PROJECT_MANAGEMENT/01_ROADMAPS_AND_PLANS/00_MASTER_ROADMAP_JONA.md` | ✅ **Ready** | "What we are building next" (Project Management).     |
| **Cap Table/Ask**     | `01_INVESTOR_PACKAGE/01_pitch_materials/FUNDING_PROPOSAL.md`                  | ✅ **Ready** | Specifics on the raise and use of funds.              |

---

## 📣 General Assets (The "Gloss")

| Asset Type          | Status | Description                                                     |
| :------------------ | :----- | :-------------------------------------------------------------- |
| **Visual Abstract** | ❌     | `02_SCIENTIFIC_COMMUNICATION/ASSET_Design_Briefs.md` (Planned). |
| **Demo Notebook**   | ✅     | `../../../research/demo/interactive_manifold.ipynb`.            |
| **One-Pager**       | ❌     | A single PDF flyer summarizing everything.                      |
| **Stakeholder Map** | ✅     | `STAKEHOLDER_ANALYSIS.md` (Who to talk to).                     |

---

## 🚀 Recommended Next Steps

1.  **Generate the `TECHNICAL_WHITEPAPER.md`**: Consolidate your fragmented theory notes into a single "Source of Truth" document that looks like a paper.
2.  **Detailed `CASE_STUDY_VIRAL_ESCAPE.md`**: Write the "Hero Story" of predicting Covid variants.
3.  **Draft `PITCH_DECK_SCRIPT.md`**: Convert `PITCH.md` into a spoken narrative for investors.
