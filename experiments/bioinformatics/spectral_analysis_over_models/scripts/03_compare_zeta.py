...
ZETA_ZEROS_100 = np.array([
    ...
    88.809111207634465423, 92.491899270558484297, 94.651344040519886966,
    95.870634228245309758, 98.831194218193692234, 101.31785100573139122,
    ...
    116.22668032085755438, 118.79078286597621733, 121.37012500242064591,
    ...
    147.42276534919928971, 150.05352042078395757, 150.92525761645737443,
    153.02469388478088856, 156.11290929488542804, 157.59759166389549466,
    158.84998819391903457, 161.18896413047820800, 163.03070968662348549,
    165.53706942685091296, 167.18443992563737322, 169.09451541574791865,
    169.91197647941169896, 173.41153628461824611, 174.75419152917556958,
    176.44143386338821657, 178.37740777609098423, 179.91648402025700898,
    182.20707848436646108, 184.87446784742265304, 185.59878367769787686,
    187.22892258142234542, 189.41615865959812894, 192.02665636781364659,
    193.07972660330851588, 195.26539668143130628, 196.87648174678348640,
    198.01530956325059576, 201.26475194370446731, 202.49359453678860297,
    204.18967180042754564, 205.39469720928828530, 207.90625889011885648,
    209.57650969649856361, 211.69086259164145329, 213.34791935629091578,
    214.54704478393905007, 216.16953848274682713, 219.06759635718080498,
    220.71491885794096567, 221.43070548923461542, 224.00700025328681230,
    224.98332466958052543, 227.42144501747689228, 229.33741330869986678,
    231.25018870043791927, 231.98723519112696132, 233.69340355929794530,
])
...
def compare_spectra(eigenvalues: np.ndarray, zeta_zeros: np.ndarray,
    ...
    eigen_cdf_interp = np.interp(common_grid,
                                  np.sort(eigen_spacings),
                                  np.arange(len(eigen_spacings)) / len(eigen_spacings))
    zeta_cdf_interp = np.interp(common_grid,
    ...
    ax1 = axes[0, 0]
    ax1.hist(eigen_spacings, bins=30, density=True, alpha=0.6,
             label='Laplacian eigenvalues', color='steelblue')
    ax1.hist(zeta_spacings, bins=30, density=True, alpha=0.6,
             label='Zeta zeros', color='crimson')
    ...
    ax2.plot([0, max(eigen_spacings.max(), zeta_spacings.max())],
...
def compute_pair_correlation(spacings: np.ndarray, output_dir: Path = None) -> dict:
    ...
    if output_dir:
        ...
        plt.plot(x, montgomery, 'r-', linewidth=2,
    ...
    montgomery_at_bins = 1 - (np.sin(np.pi * bin_centers + 1e-10) /
                              (np.pi * bin_centers + 1e-10))**2
...
def main():
    ...
    parser.add_argument('--n-zeta-zeros', type=int, default=100,
