...
from hyperbolic_utils import (
    load_codon_encoder, codon_to_onehot, AA_TO_CODON,
...
def main():
    ...
    for i, (x, y) in enumerate(embeddings_disk):
        ...
        ax.annotate(all_labels[i], (x, y), fontsize=7, alpha=0.7,
    ...
    for name, data in approach_centroids.items():
        ...
        ax.annotate(data['title'], (x, y), fontsize=9, fontweight='bold',
    ...
    ax.set_ylim(-1.1, 1.1)
    ax.set_aspect('equal')
    ax.set_title('HIV Treatment Approaches in Hyperbolic Codon Space\n(3-adic Encoder, Poincare Disk Projection)', fontsize=14)
