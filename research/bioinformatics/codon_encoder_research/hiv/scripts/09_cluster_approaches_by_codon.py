...
from hyperbolic_utils import (
    load_codon_encoder, poincare_distance, poincare_distance_matrix,
    codon_to_onehot, AA_TO_CODON, hyperbolic_centroid, hyperbolic_variance
)
...
def encode_mutation(encoder, wt_aa, mut_aa):
    ...
    dist = poincare_distance(
        ...
        torch.tensor(mut_emb).unsqueeze(0)
    ).item()
