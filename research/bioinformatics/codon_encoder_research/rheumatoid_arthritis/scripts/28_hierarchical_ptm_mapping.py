...
def extract_cluster_hierarchy(encoder: CodonEncoder, device: str = 'cpu') -> Dict:
    ...
    def get_tree_depth(node, depth=0):
        if node.is_leaf():
            return depth
        return max(get_tree_depth(node.left, depth + 1),
...
def compute_hierarchical_goldilocks(
    ...
    return {
        ...
        'pair_rate': len(pair_goldilocks) / (len(pair_hierarchy['same_branch_pairs']) + len(pair_hierarchy['cross_branch_pairs'])) if pair_hierarchy['same_branch_pairs'] or pair_hierarchy['cross_branch_pairs'] else 0,
    }
...
def generate_hierarchy_visualizations(
    ...
    for i, ptm in enumerate(ptm_names):
        ...
        ax.annotate('', xy=(to_c, i), xytext=(from_c, i),
    ...
    bars1 = ax.bar(x - width/2, distances, width, label='Embedding Distance', color='steelblue', alpha=0.7)
    bars2 = ax.bar(x + width/2, js_divs, width, label='JS Divergence', color='coral', alpha=0.7)

    # Add divergence level as text
    for i, (bar, level) in enumerate(zip(bars1, divergence_levels)):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
                f'L{level}', ha='center', va='bottom', fontsize=8)

    ax.set_xticks(x)
    ax.set_xticklabels([f"{p.split('→')[0]}→{p.split('→')[1]}" for p in sorted_ptms],
    ...
    plt.suptitle('Hierarchical PTM Space Mapping', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_dir / 'hierarchical_ptm_mapping.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def generate_ultrametric_tree(
    ...
    dend = dendrogram(linkage_mat, ax=ax, labels=labels, leaf_rotation=45,
    ...
    for ptm_name, t in transitions.items():
        ...
        if from_c != to_c:
            ...
            if t['embedding_distance'] > 0.3:
                ax.annotate(ptm_name, xy=(0.02, 0.98 - list(transitions.keys()).index(ptm_name) * 0.04),
    ...
    ax.set_title('Ultrametric Codon Cluster Tree\nwith PTM Transition Annotations', fontsize=12, fontweight='bold')
...
def main():
    ...
    for ptm_name, t in sorted(transitions.items(), key=lambda x: -x[1]['embedding_distance']):
        print(f"    {ptm_name}: C{t['from_cluster']}→C{t['to_cluster']}, "
    ...
    if goldilocks['pair_goldilocks']:
        ...
        for pg in sorted(goldilocks['pair_goldilocks'], key=lambda x: x['combined_estimate'])[:5]:
            print(f"    {pg['ptm1']} + {pg['ptm2']}: est={pg['combined_estimate']:.2f}, "
    ...
    results = {
        ...
        'key_findings': {
            ...
            'goldilocks_optimal_pairs': [
                f"{g['ptm1']}+{g['ptm2']}"
                for g in goldilocks['pair_goldilocks']
            ][:10],
        }
    }
