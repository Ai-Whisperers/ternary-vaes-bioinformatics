...
def analyze_known_acpa_pattern(samples: list) -> dict:
    ...
    analysis = {"total_known": len(known), "sites": []}
...
def main():
    ...
    for ptm in ["R->Q", "S->D", "M->Q", "T->D"]:
        if ptm in amplification:
            a = amplification[ptm]
            print(
                f"  {ptm}: global={a['simple_shift_mean']:.4f} -> TCR={a['tcr_shift_mean']:.4f} "
    ...
    for site in known["sites"]:
        print(
            f"  {site['protein']} R{site['position']}: score={site['interface_score']}/3 "
