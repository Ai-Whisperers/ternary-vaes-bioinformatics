...
def main():
    ...
    for protein_name, epitope_list in auto["analyses"].items():
        for ep in epitope_list:
            ...
            if arg_data:
                epitope_metrics[name]["embedding_norm"] = np.mean(
                    [a.get("embedding_norm", np.nan) for a in arg_data]
                )
                epitope_metrics[name]["mean_neighbor_dist"] = np.mean(
                    [a.get("mean_neighbor_distance", np.nan) for a in arg_data]
                )
                epitope_metrics[name]["boundary_potential"] = np.mean(
                    [a.get("boundary_crossing_potential", np.nan) for a in arg_data]
                )
            epitope_metrics[name]["immunodominant"] = (
                1 if ep.get("immunodominant", False) else 0
            )
    ...
    for ep_data in shift["all_shifts"]:
        ...
        if name in epitope_metrics and arg_shifts:
            epitope_metrics[name]["mean_shift"] = np.mean(
                [float(s["centroid_shift"]) for s in arg_shifts]
            )
            epitope_metrics[name]["mean_js"] = np.mean(
                [float(s["js_divergence"]) for s in arg_shifts]
            )
            epitope_metrics[name]["mean_entropy"] = np.mean(
                [float(s["entropy_change"]) for s in arg_shifts]
            )
    ...
    for metric, s in auto_stats.items():
        if isinstance(s, dict) and "p_value" in s and s["p_value"] < 0.05:
            direction = (
                "lower"
                if s.get("immunodominant_mean", 0) < s.get("silent_mean", 0)
                else "higher"
            )
    ...
    for f in significant_findings:
        effect_str = (
            f"{f['effect']:.2f}" if isinstance(f["effect"], float) else f["effect"]
        )
        print(
            f"{f['source']:<12} {f['metric']:<30} {f['p_value']:<10.4f} {f['direction']:<25} {effect_str:<10}"
        )
    ...
    agreement_rate = (
        immuno_agree / (immuno_agree + immuno_disagree)
        if (immuno_agree + immuno_disagree) > 0
        else 0
    )
    conclusions.append(
        f"Immunodominance classification: {agreement_rate*100:.0f}% agreement across analyses"
    )

    # 2. Shift-immunodominance relationship
    if r_shift_immuno < 0 and p_shift_immuno < 0.1:
        conclusions.append(
            f"Immunodominant epitopes show SMALLER shifts (r={r_shift_immuno:.3f}, p={p_shift_immuno:.3f})"
        )

    # 3. Boundary crossing
    bc_rate = cit["n_boundary_crossed"] / cit["n_with_cit_site"]
    conclusions.append(
        f"Boundary crossing rate: {bc_rate*100:.1f}% (citrullination rarely changes cluster)"
    )

    # 4. Statistical convergence
    conclusions.append(f"{len(significant_findings)} independent metrics show p<0.05")
    conclusions.append(
        f"Direction consistency: {lower_count}/{len(directions)} metrics show immunodominant LOWER"
    )

    # 5. HLA geometry
    conclusions.append(
        f"HLA-DRB1 RA alleles: {hla['separation_ratio']:.2f}x separation ratio"
    )
