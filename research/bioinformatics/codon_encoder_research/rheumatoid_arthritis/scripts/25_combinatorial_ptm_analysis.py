...
def plot_synergy_analysis(results: List[Dict], output_dir: Path):
    ...
    ax = axes[0, 0]
    ax.scatter(expected_shifts, combined_shifts, c=distances, cmap='viridis',
    ...
    ax.set_title('Synergy Analysis: Combined vs Expected', fontsize=12, fontweight='bold')
    ...
    ax.axvline(0.9, color='orange', linestyle='--', lw=1, label='Antagonistic threshold')
    ...
    ax = axes[1, 0]
    ax.scatter(distances, synergy_ratios, alpha=0.6, s=50, c='steelblue', edgecolors='white')
    ...
    ax.set_ylabel('Count', fontsize=12)
    ax.set_title(f'Goldilocks Zone Analysis (n={len(results)} pairs)', fontsize=12, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')

    plt.suptitle('Combinatorial PTM Analysis: Citrullination + Deglycosylation',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_dir / 'combinatorial_synergy_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_top_synergistic_pairs(results: List[Dict], output_dir: Path, top_n: int = 15):
    ...
    bars1 = ax.bar(x - width, r_shifts, width, label='R→Q alone', color='#e53935', alpha=0.7)
    bars2 = ax.bar(x, n_shifts, width, label='N→Q alone', color='#1e88e5', alpha=0.7)
    bars3 = ax.bar(x + width, combined_shifts, width, label='Combined', color='#9c27b0', alpha=0.7)
...
def main():
    ...
    if potentiation:
        ...
        for p in potentiation[:10]:
            print(f"    {p['pair']['protein']}: R{p['pair']['r_position']}-N{p['pair']['n_position']} "
    ...
    for res in sorted_by_synergy[:5]:
        print(f"    {res['pair']['protein']}: R{res['pair']['r_position']}-N{res['pair']['n_position']} "
