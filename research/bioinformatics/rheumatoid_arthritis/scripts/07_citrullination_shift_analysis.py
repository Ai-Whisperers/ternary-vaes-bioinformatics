...
EPITOPE_DATABASE = [
    # Vimentin
    {'id': 'VIM_R71', 'seq': 'RLRSSVPGVR', 'arg_pos': [0, 2, 9], 'immunodominant': True, 'acpa': 0.85},
    {'id': 'VIM_R257', 'seq': 'SSLNLRETNL', 'arg_pos': [5], 'immunodominant': True, 'acpa': 0.72},
    {'id': 'VIM_R45', 'seq': 'SSRSFRTYSF', 'arg_pos': [2, 5], 'immunodominant': False, 'acpa': 0.15},
    {'id': 'VIM_R201', 'seq': 'ARLRSSLAGS', 'arg_pos': [0, 2], 'immunodominant': True, 'acpa': 0.68},
    # Fibrinogen
    {'id': 'FGA_R38', 'seq': 'GPRVVERHQS', 'arg_pos': [2, 6], 'immunodominant': True, 'acpa': 0.78},
    {'id': 'FGA_R573', 'seq': 'MELERPGGNEI', 'arg_pos': [4], 'immunodominant': True, 'acpa': 0.65},
    {'id': 'FGA_R84', 'seq': 'RHPDEAAFFDT', 'arg_pos': [0], 'immunodominant': False, 'acpa': 0.22},
    {'id': 'FGB_R74', 'seq': 'HARPAKAATN', 'arg_pos': [2], 'immunodominant': True, 'acpa': 0.71},
    {'id': 'FGB_R44', 'seq': 'NEEGFFRHNDK', 'arg_pos': [7], 'immunodominant': False, 'acpa': 0.18},
    # Alpha-enolase
    {'id': 'ENO1_CEP1', 'seq': 'KIREEIFDSRGNP', 'arg_pos': [2, 9], 'immunodominant': True, 'acpa': 0.62},
    {'id': 'ENO1_R400', 'seq': 'SFRSGKYKSV', 'arg_pos': [2], 'immunodominant': False, 'acpa': 0.12},
    # Collagen II
    {'id': 'CII_259', 'seq': 'GARGLTGRPGDAGK', 'arg_pos': [2, 7], 'immunodominant': True, 'acpa': 0.45},
    {'id': 'CII_511', 'seq': 'PGERGAPGFRGPAG', 'arg_pos': [3, 10], 'immunodominant': True, 'acpa': 0.38},
    # Filaggrin
    {'id': 'FLG_CCP', 'seq': 'SHQESTRGRS', 'arg_pos': [6, 8], 'immunodominant': True, 'acpa': 0.75},
    {'id': 'FLG_SEC', 'seq': 'DSHRGSSSSS', 'arg_pos': [3], 'immunodominant': False, 'acpa': 0.20},
    # Histones
...
def compute_epitope_shift_profile(epitope, encoder, device='cpu'):
    ...
    for arg_idx in epitope['arg_pos']:
        ...
        js_div = 0.5 * np.sum(original_cluster_dist * np.log((original_cluster_dist + 1e-10) / (m + 1e-10))) + \
                 0.5 * np.sum(cit_cluster_dist * np.log((cit_cluster_dist + 1e-10) / (m + 1e-10)))
    ...
    for result in all_shifts:
        ...
        for shift in result['arg_shifts']:
            for key, value in shift.items():
                if key not in ['position', 'arg_cluster', 'max_gain_cluster', 'max_loss_cluster']:
...
def main():
    ...
    for metric, stats_data in comparisons.items():
        sig = "***" if stats_data['p_value'] < 0.001 else \
    ...
    if 'js_divergence' in comparisons:
        ...
        print(f"  The cluster probability distribution changes {'less' if js['imm_mean'] < js['sil_mean'] else 'more'} "
        ...
        'all_shifts': [
            {
                **r,
                'arg_shifts': [
                    {k: float(v) if isinstance(v, np.floating) else v for k, v in s.items()}
                    for s in r['arg_shifts']
                ]
            }
            for r in all_shifts if r
        ],
        'comparisons': {
            k: {kk: float(vv) if isinstance(vv, np.floating) else vv for kk, vv in v.items()}
