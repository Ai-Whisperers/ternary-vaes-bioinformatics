...
def create_visualization(alleles_embeddings, allele_data, results, output_path):
    ...
    colors = ['red' if allele_data[name]['ra_status'] == 'risk' else 'blue'
              for name in allele_names]

    ax1.scatter(centroids_2d[:, 0], centroids_2d[:, 1], c=colors, s=150, alpha=0.7)
    for i, name in enumerate(allele_names):
        ax1.annotate(name.split('*')[1], (centroids_2d[i, 0], centroids_2d[i, 1]),
    ...
    ax2.set_xticklabels([n.split('*')[1] for n in allele_names], rotation=45, ha='right', fontsize=8)
    ...
    if sep['between_mean'] > 0:
        ...
        ax3.text(0.5, 0.95, f'Between/Within Ratio: {ratio:.2f}x',
...
def main():
    ...
    for dist_fn in ['euclidean', 'poincare', 'ultrametric']:
        ...
        max_pos = max(pos_results.keys(), key=lambda p: pos_results[p]['centroid_distance'])
    ...
    best_fn = max(results.keys(), key=lambda k: results[k]['separation']['between_mean'] /
