...
class BoundaryAnalyzer:
    ...
    def simulate_citrullination(self, codons: List[str], r_position: int) -> Dict:
        ...
        embeddings = [self.codon_embeddings[c] for i, c in enumerate(codons)
...
class CodonOptimizer:
    ...
    def optimize(self, protein_sequence: str,
        ...
        while improved and iteration < max_iterations:
            ...
            for i in range(n):
                ...
                best_score = self._score_position(current_codons, i, weight_margin,
                                                   weight_usage, weight_cit_safety)
                ...
                for codon in synonymous:
                    ...
                    score = self._score_position(test_codons, i, weight_margin,
                                                  weight_usage, weight_cit_safety)
    ...
    def _score_position(self, codons: List[str], position: int,
                        weight_margin: float, weight_usage: float,
                        weight_cit_safety: float) -> float:
        ...
        for start in range(max(0, position - self.epitope_window + 1),
...
def create_visualization(results: Dict, output_path: Path):
    ...
    ax1.axhline(y=results['metrics']['mean_margin'], color='red', linestyle='--',
                label=f'Mean: {results["metrics"]["mean_margin"]:.3f}')
    ax1.axhline(y=results['metrics']['min_margin'], color='orange', linestyle=':',
    ...
    if results['citrullination_analysis']:
        ...
        ax2.set_title(f'Citrullination Sensitivity at R Positions\n'
                     f'(Red=Boundary Crossed, Green=Safe)')

        # Add safety rate
        safety_rate = results['metrics']['cit_safety_rate']
        ax2.text(0.5, 0.95, f'Safety Rate: {safety_rate:.1%}',
    ...
    for i, codon in enumerate(results['codons']):
        if codon in HUMAN_CODON_USAGE:
            ...
            margins_at_pos = [w['margin'] for w in results['epitope_windows']
                             if w['start'] <= i < w['end']]
            if margins_at_pos:
...
def main():
    ...
    for key, protein in SYNOVIAL_PROTEINS.items():
        ...
        print(f"    Mean Margin: {result['metrics']['mean_margin']:.4f} "
