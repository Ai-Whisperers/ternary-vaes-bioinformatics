...
def build_graph_laplacian(distances: torch.Tensor, sigma: float = 1.0,
                          k_neighbors: int = None) -> torch.Tensor:
...
def analyze_spacing_distribution(spacings: np.ndarray, output_dir: Path):
    ...
    # Plot histogram
    plt.hist(spacings_norm, bins=50, density=True, alpha=0.7,
             label='Observed spacings', color='steelblue', edgecolor='black')

    # Plot theoretical distributions
    plt.plot(s_range, gue_distribution(s_range), 'r-', linewidth=2,
             label='GUE (Riemann zeta zeros)')
    plt.plot(s_range, goe_distribution(s_range), 'g--', linewidth=2,
             label='GOE')
    plt.plot(s_range, poisson_distribution(s_range), 'b:', linewidth=2,
