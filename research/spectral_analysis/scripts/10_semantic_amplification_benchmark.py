...
def method_geometric(radii: np.ndarray, indices: np.ndarray,
...
def find_optimal_threshold(radii: np.ndarray, valuations: np.ndarray,
                           target_valuation: int) -> tuple:
...
def method_radial_shells(radii: np.ndarray, indices: np.ndarray,
                         boundaries: dict, target_valuation: int) -> tuple:
    """
...
def run_benchmark():
    ...
    for v in sorted(boundaries.keys()):
        b = boundaries[v]
        print(f"  v_3 = {v}: r ∈ [{b['r_min']:.4f}, {b['r_max']:.4f}], "
