...
def visualize_results(model, data, vae_embeddings, eval_results, output_dir):
    ...
    for i, cluster_id in enumerate(unique_clusters):
        ...
        ax1.scatter(coords_2d[mask, 0], coords_2d[mask, 1],
                   c=[colors[i]], s=80, alpha=0.7, label=f'C{cluster_id}')
    ...
    ax2.scatter(natural_2d[:, 0], natural_2d[:, 1], c='gray', s=50, alpha=0.5, label='VAE natural')
    ax2.scatter(coords_2d[:, 0], coords_2d[:, 1], c='red', s=30, alpha=0.7, label='Learned')
    ...
    ax3.text(0.5, 0.5, 'Training complete\nCluster acc: {:.1f}%'.format(
        eval_results['cluster_acc'] * 100), ha='center', va='center', fontsize=14)
